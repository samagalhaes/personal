...
begin
  ...
  // Desaceleracao a uma taxa DVMAX
    v := v - DVMAX;
    if (v <= 0) then
      v := 0;
    
    // Aceleracao a uma taxa DWMAX
    vw := 1*sign(vw)*DWMAX + vw;
    if (abs(vw) >= WNOM) then
  ...
  2: begin // Move para a posicao final
    ...
    // Aceleracao a uma taxa DVMAX
    v := v + DVMAX;
    if (v >= VNOM) then
      v := VNOM;

    // Desaceleracao a uma taxa DWMAX
    vw := vw - DWMAX*sign(vw);
    if (abs(vw) <= abs(GAIN_FWD*(theta-aux_theta)) ) then
  ...
  3: begin // Desacelaracao da velocidade linear ao aproximar da posicao final
    ...
    // Desaceleracao a uma taxa DWMAX
    vw := vw - DWMAX*sign(vw);
    if (abs(vw) <= abs(GAIN_DA*(theta-aux_theta)) ) then
  ...
  5: begin // Desaceleracao da velocidade angular ao aproximar de theta final
    ...
    if (v <= 0) then
      v := 0;
       ...
       // Desaceleracao a uma taxa DWMAX
    vw := vw - DWMAX*sign(vw);
    if (abs(vw) <= abs(W_DA * (thetaf-theta)) ) then
